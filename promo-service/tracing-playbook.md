...
2. **Hit the wall** — Grafana → Explore → Tempo:
   `{ span.http.route = "/checkout" && duration > 500ms }`
   Slow traces are 3 spans; no attribute says which requests or why.

3. **Apply TRACING STEP 1–5** — each comment block names the active lines
...
5. **Re-trace** — rerun the step-2 query (search lags ingestion ~1min).
...
6. **Resolve** — disable the flag; p99 on the payment dashboard collapses
   within ~5min. Batch path (`fetch_rules_batch`) is the permanent fix.

7. **Reset for next cohort** — `git checkout -- promo-service/`, then
   repeat step 4.
